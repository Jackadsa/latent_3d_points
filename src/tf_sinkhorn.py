...
#     vprev = tf.fill(tf.shape(b),np.float64(1.))/tf.reduce_sum(b,axis=-1,keepdims=True)
    ...
    def loopfn(flag, err, cpt, u, v, uprev, vprev, K, reg, cpt_outer):
        ...
        cptmod_cond = tf.logical_and(tf.equal(tf.floormod(cpt,numItermaxinner),0),
                                    tf.not_equal(cpt,0))
        def cptmod_true():
...
#         return tf.transpose(loss_grad_temp,[0,2,1])  
    ...
    return tf.trace(tf.matmul(tf.cast(match,tf.float32),ground_distance,transpose_b=True))
