...
def variational_encoder_with_convs_and_symmetry(in_signal, n_filters=[64, 128, 256, 1024], filter_sizes=[1], strides=[1],
    ...
    if EFN:
        in_pt = in_signal[:,:,0]
        in_coordinates = in_signal[:,:,1:]
    else:
        in_coordinates = in_signal
    ...
    for i in xrange(n_layers):
        ...
        if b_norm and b_norm is not "after":
            name += '_bnorm'
            scope_i = expand_scope_by_name(scope, name)
...
#                 print 'bnorm params = ', np.prod(layer.beta.get_shape().as_list()) + np.prod(layer.gamma.get_shape().as_list())
    ...
    if symmetry is not None:
        if EFN:
            layer = tf.expand_dims(in_pt,-1)*layer
...
#                     print 'bnorm params = ', np.prod(layer.beta.get_shape().as_list()) + np.prod(layer.gamma.get_shape().as_list())
    ...
    def sampling(args):
        ...
        if rescale_logvar:
            ...
            logvar_rescaled = tf.where(logvar_cond, z_log_var + 1., K.exp(0.5 * z_log_var))
    ...
    if (fully_connected_layers is not None) and (reshape_last_connected is not None):
        for i, layer_size in enumerate(fully_connected_layers):
            ...
            if b_norm:
                ...
                scope_i = expand_scope_by_name(scope, name)
                #layer = batch_normalization(layer, name=name, reuse=reuse, scope=scope_i)
                layer = tf.keras.layers.BatchNormalization(axis=-1,renorm=True,name=name, momentum=bnorm_momentum,renorm_momentum=bnorm_momentum,epsilon=1e-5)(layer)
#                 if verbose:
#                     print 'bnorm params = ', np.prod(layer.beta.get_shape().as_list()) + np.prod(layer.gamma.get_shape().as_list())
...
#                     print 'bnorm params = ', np.prod(layer.beta.get_shape().as_list()) + np.prod(layer.gamma.get_shape().as_list())
    ...
    for i in xrange(n_layers):
        ...
        if ((b_norm and i < n_layers - 1) or (i == n_layers - 1 and b_norm_finish)) and b_norm is not "after":
            name += '_bnorm'
            scope_i = expand_scope_by_name(scope, name)
            #layer = batch_normalization(layer, name=name, reuse=reuse, scope=scope_i)
            layer = tf.keras.layers.BatchNormalization(axis=-1,
...
#                 print 'bnorm params = ', np.prod(layer.beta.get_shape().as_list()) + np.prod(layer.gamma.get_shape().as_list())
        ...
        if ((b_norm and i < n_layers - 1) or (i == n_layers - 1 and b_norm_finish)) and b_norm is "after":
            name += '_bnorm'
            scope_i = expand_scope_by_name(scope, name)
            #layer = batch_normalization(layer, name=name, reuse=reuse, scope=scope_i)
            layer = tf.keras.layers.BatchNormalization(axis=-1,
