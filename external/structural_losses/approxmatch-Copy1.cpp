...
int main()
{
	...
	float * xyz1=new float[b*n*2];
	float * xyz2=new float[b*m*2];
	float * match=new float[b*n*m];
	float * match_cpu=new float[b*n*m];
	float * cost=new float[b];
	float * cost_cpu=new float[b];
	float * grad=new float[b*m*2];
	float * grad_cpu=new float[b*m*2];
	...
	float * xyz1_g;
	cudaMalloc(&xyz1_g,b*n*2*4);
	float * xyz2_g;
	cudaMalloc(&xyz2_g,b*m*2*4);
	float * match_g;
	cudaMalloc(&match_g,b*n*m*4);
	float * cost_g;
	cudaMalloc(&cost_g,b*n*2*4);
	float * grad_g;
	cudaMalloc(&grad_g,b*m*2*4);

	cudaMemcpy(xyz1_g,xyz1,b*n*2*4,cudaMemcpyHostToDevice);
	cudaMemcpy(xyz2_g,xyz2,b*m*2*4,cudaMemcpyHostToDevice);
	cudaMemset(match_g,0,b*n*m*4);
	cudaMemset(cost_g,0,b*4);
	cudaMemset(grad_g,0,b*m*2*4);
